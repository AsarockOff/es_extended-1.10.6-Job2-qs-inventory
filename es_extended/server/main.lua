...
AddEventHandler("playerDropped", function(reason)
    ...
    if xPlayer then
		...
		local currentJob2 = ESX.Jobs2PlayerCount[job2]
        ESX.JobsPlayerCount[job] = ((currentJob and currentJob > 0) and currentJob or 1) - 1
		ESX.Jobs2PlayerCount[job2] = ((currentJob2 and currentJob2 > 0) and currentJob2 or 1) - 1
        GlobalState[("%s:count"):format(job)] = ESX.JobsPlayerCount[job]
		GlobalState[("%s:count2"):format(job2)] = ESX.Jobs2PlayerCount[job2]
...
AddEventHandler("esx:playerLoaded", function(_, xPlayer)
	...
	local job2Key = ("%s:count2"):format(job2)

    ESX.JobsPlayerCount[job] = (ESX.JobsPlayerCount[job] or 0) + 1
	ESX.Jobs2PlayerCount[job2] = (ESX.Jobs2PlayerCount[job2] or 0) + 1
    GlobalState[jobKey] = ESX.JobsPlayerCount[job]
	GlobalState[job2Key] = ESX.Jobs2PlayerCount[job2]
end)
...
AddEventHandler("esx:setJob2", function(_, job2, lastJob)
    local lastJobKey = ("%s:count2"):format(lastJob.name)
    local jobKey = ("%s:count2"):format(job2.name)
    local currentLastJob = ESX.Jobs2PlayerCount[lastJob.name]

    ESX.Jobs2PlayerCount[lastJob.name] = ((currentLastJob and currentLastJob > 0) and currentLastJob or 1) - 1
    ESX.Jobs2PlayerCount[job2.name] = (ESX.Jobs2PlayerCount[job2.name] or 0) + 1

    GlobalState[lastJobKey] = ESX.Jobs2PlayerCount[lastJob.name]
    GlobalState[jobKey] = ESX.Jobs2PlayerCount[job2.name]
end)
