...
function ESX.GetNumPlayers(key, val)
    ...
    if type(val) == "table" then
        ...
        if key == "job2" then
            for _, v in ipairs(val) do
                numPlayers[v] = (ESX.Jobs2PlayerCount[v] or 0)
            end
            return numPlayers
        end
	...
	if key == "job2" then
        return (ESX.Jobs2PlayerCount[val] or 0)
    end
...
function ESX.CreateJob2(name, label, grades)
    ...
    for _, v in pairs(grades) do
        job.grades[tostring(v.grade)] = { job2_name = name, grade = v.grade, name = v.name, label = v.label, salary = v.salary, skin_male = v.skin_male or "{}", skin_female = v.skin_female or "{}" }
        parameters[#parameters + 1] = { name, v.grade, v.name, v.label, v.salary, v.skin_male or "{}", v.skin_female or "{}" }
    end

    MySQL.insert("INSERT IGNORE INTO jobs2 (name, label) VALUES (?, ?)", { name, label })
    MySQL.prepare("INSERT INTO job2_grades (job2_name, grade, name, label, salary, skin_male, skin_female) VALUES (?, ?, ?, ?, ?, ?, ?)", parameters)
...
function ESX.RefreshJobs()
    ...
    for _, v in ipairs(job2Grades) do
        if Jobs2[v.job2_name] then
            ...
            print(('[^3WARNING^7] Ignoring job grades for ^5"%s"^0 due to missing job2'):format(v.job2_name))
        end
    end
    ...
    for _, v in pairs(Jobs2) do
        if ESX.Table.SizeOf(v.grades) == 0 then
            Jobs2[v.name] = nil
            print(('[^3WARNING^7] Ignoring job ^5"%s"^0 due to no job2 grades found'):format(v.name))
        end
    end
    --jobs
    if not Jobs then
    ...
    if not Jobs2 then
        -- Fallback data, if no jobs exist
        ESX.Jobs2["unemployed"] = { label = "Unemployed", grades = { ["0"] = { grade = 0, label = "Unemployed", salary = 0, skin_male = {}, skin_female = {} } } }
    else
        ESX.Jobs2 = Jobs2
    end
end
